...
class OpenAccountChart(models.TransientModel):
	...
	def onchange_company(self):
		if not self.env['account.account'].search([('parent_id', '!=', False)], limit=1):
	...
	def account_chart_open_window(self):
		...
		result = {
			...
			'tag': 'coa_hierarchy',
			'context': """{'url': '/account_parent/output_format/account_parent?active_id=%s&amp;active_model=%s',
									'model': 'account.open.chart',
									'active_model': 'account.open.chart'}""" % (self.id, 'account.open.chart')
	...
	def format_amount_to_str(self, value, currency):
		...
		formatted_amount = lang.format(fmt, currency.round(value),
	...
	def at_line_data(self, at_data, level, wiz_id=False, parent_id=False, accounts=False):
		...
		data.update({
			...
			'company_obj': company,
			'debit': '%s' % self._float_html_formating(total_debit, company),
...
# 		lines = self.final_vals_to_lines(final_vals, level, html_formating)
	...
	def _get_html(self):
		...
		lines = self.with_context(user_context).get_lines(wiz_id=user_context.get('active_id'))
		heading = self.get_heading(user_context)
	...
	def get_html(self, given_context=None):
		res = self.browse()
		if given_context.get('active_id') and given_context.get('active_model') == 'account.open.chart':
			res = self.browse(given_context.get('active_id'))
		# if not res:
		# 	res = self.search([('create_uid', '=', self.env.uid)], order="id desc", limit=1)
		# 	print(res,'searchrrrrr')
		# if not res:
		# 	res = self.create({})
		# 	print(res, 'createeeeeeeeeee')
	...
	def show_journal_items(self, wiz_id=None, line_id=None, acc_name=''):
		...
		if report_type == 'account_type':
			at_datas = list(filter(lambda x: x['parent_id'] == line_id, self.account_type_data()))
			for at_data in at_datas:
				account_ids += self.get_at_accounts(at_data, context).ids
			if not at_datas:
				at_datas = list(filter(lambda x: x['id'] == line_id, self.account_type_data()))
				for at_data in at_datas:
					account_ids += self.get_at_accounts(at_data, context).ids
			...
			account_ids += self.get_accounts(line_id, context).ids
		domain, context = self.build_domain(wiz_id=wiz_id, account_ids=account_ids, context=context)
		return {
