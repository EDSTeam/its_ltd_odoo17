...
class CoAReportController(http.Controller):
    ...
    @http.route('/account_parent/<string:output_format>/<string:report_name>', type='http', auth='user')
    def report(self, output_format, report_name=False, **kw):
        uid = request.session.uid
        try:
            if output_format == 'pdf':
                report_id = int(kw['active_id'])
                ...
                report_id = int(kw['active_id'])
                report_obj = request.env['account.open.chart'].with_user(uid).browse(report_id)
